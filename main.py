...
# ============================================================
# EXISTING TRAINED MODEL (UNTOUCHED)
# ============================================================
...
def download_stock_data(stock, start=None, end=None):
    ...
    # OPTIMIZATION: 2-year window provides >500 trading days (plenty for EMA 200 & 100-day LSTM)
    # while drastically cutting Yahoo Finance fetch time from ~10s down to ~1s.
...
def prepare_test_data(df):
    if len(df) < 120:
        raise ValueError("Not enough historical data for sequence calculations.")
    ...
    if x_test.shape[0] == 0:
        raise ValueError("Insufficient test dataset length.")
    ...
    test_dates = (
        pd.to_datetime(df["Date"].iloc[training_end:])
        .dt.strftime("%Y-%m-%d")
        .tolist()
    )
...
def future_forecast(df):
    if len(df) < 100:
        raise ValueError("At least 100 closing prices required.")
    ...
    for _ in range(15):
        ...
        next_scaled = float(
            np.asarray(model.predict(x_input, verbose=0)).reshape(-1)[0]
        )
        future_scaled.append(next_scaled)
        sequence = np.append(sequence, next_scaled)

    future_prices = full_scaler.inverse_transform(
    ...
    while len(future_dates) < 15:
        ...
        if candidate.weekday() < 5:
            future_dates.append(candidate.strftime("%Y-%m-%d"))
...
def analyze_stock(stock, static_dir=None):
    ...
    if df.empty:
        raise ValueError("No stock data found for ticker symbol.")
    ...
    if static_dir:
        generate_charts(df, y_test, y_pred, test_dates, static_dir)
